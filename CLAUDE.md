...
- **Resize rule**: only downscale when a dimension exceeds `MAX_DIM`; scale factor is `Math.min(MAX_DIM/w, MAX_DIM/h)` so aspect ratio is preserved and nothing is upscaled. `MAX_DIM` starts at 1920 (also the hard ceiling) but is now mutable — the "Max size" range slider (`#max-dim-slider`, min 320 / max 1920) reassigns it on `input` and updates two readouts (`#max-dim-val` badge and the header `#max-dim-label`). The cap is a square box, so one slider drives both sides; per-image aspect ratio is still handled by the scale-to-fit math. Read at convert time, so changing it only affects files converted afterward; already-`done` rows keep their result.
